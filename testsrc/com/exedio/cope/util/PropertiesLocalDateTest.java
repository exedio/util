...
public class PropertiesLocalDateTest
{
	@Test void testMinimal()
	{
		...
		assertEquals(of(1000, 8, 31), props.mandatory);
		assertEquals(of(1000, 8, 31), props.mandatoryF.getValue());
		assertEquals(of(1000, 8, 31), props.mandatoryF.get());
		assertEquals(of(1009, 7, 13), props.optional);
		assertEquals(of(1009, 7, 13), props.optionalF.getValue());
		assertEquals(of(1009, 7, 13), props.optionalF.get());
	...
	@Test void testSet()
	{
		...
		p.setProperty("mandatory", "2000-11-04");
		p.setProperty("optional", "2001-03-31");
		final MyProps props = new MyProps(p);
		props.assertIt();

		assertEquals(of(2000, 11,  4), props.mandatory);
		assertEquals(of(2000, 11,  4), props.mandatoryF.getValue());
		assertEquals(of(2000, 11,  4), props.mandatoryF.get());
		assertEquals(of(2001,  3, 31), props.optional);
		assertEquals(of(2001,  3, 31), props.optionalF.getValue());
		assertEquals(of(2001,  3, 31), props.optionalF.get());
	...
	@Test void testMandatoryWrong()
	{
		assertWrong(
				...
				"must be a local date formatted as yyyy-mm-dd, but was 'no-date-set'",
	...
	@Test void testOptionalWrong()
	{
		assertWrong(
				...
				"must be a local date formatted as yyyy-mm-dd, but was 'no-date-set-opt'",
				DateTimeParseException.class);
	}
	...
	@Test void testMandatoryWrongYear()
	{
		assertWrong(
				"mandatory", "200-04-04",
				"must be a local date formatted as yyyy-mm-dd, but was '200-04-04'",
				DateTimeParseException.class);
	}
	...
	@Test void testMandatoryWrongMonth()
	{
		assertWrong(
				...
				"must be a local date formatted as yyyy-mm-dd, but was '2000-40-04'",
				DateTimeParseException.class);
	}
	...
	@Test void testMandatoryWrongDay()
	{
		assertWrong(
				...
				"must be a local date formatted as yyyy-mm-dd, but was '2000-04-40'",
				DateTimeParseException.class);
	}
	...
	{
		final LocalDate mandatory = value("mandatory", (LocalDate)null);
		final LocalDate optional  = value("optional", of(1009, 7, 13));
		...
		void assertIt()
		{
			...
			assertEquals(of(1009, 7, 13), optionalF .getDefaultValue());
